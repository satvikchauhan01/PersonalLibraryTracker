    ...
    <>
      ...
      <form onSubmit={handleCreateShelf} className="flex gap-2 mb-8 max-w-md">
        <input
          ...
          className="flex-grow rounded-full border border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"
        />
        <button
          type="submit"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-10">
          {shelves.map((shelf) => (
            <div
              ...
              className={`bg-white dark:bg-gray-800 rounded-xl shadow-md p-5 cursor-pointer border-2 transition-colors ${
                activeShelf?._id === shelf._id ? 'border-indigo-500' : 'border-transparent'
              } hover:border-indigo-300 dark:hover:border-indigo-600`}
            >
              <div className="flex justify-between items-start">
                <h3 className="font-bold text-gray-800 dark:text-gray-100">{shelf.name}</h3>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="inline-flex items-center px-3 py-1.5 rounded-full bg-indigo-50 dark:bg-indigo-950/50 text-indigo-700 dark:text-indigo-300 text-sm font-medium hover:bg-indigo-100 dark:hover:bg-indigo-900"
              >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {activeShelf.books.map((book) => (
                <div key={book._id} className="relative group">
                  ...
                  />
                  <p className="text-xs font-medium text-gray-700 dark:text-gray-300 mt-1 line-clamp-1">
                    {book.title}
                  </p>
                  <button
      ...
      {showAddBooks && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md max-h-[75vh] flex flex-col overflow-hidden">
            <div className="flex justify-between items-center px-5 py-4 border-b dark:border-gray-800">
